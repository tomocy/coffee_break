...
class LinkBloc {
  ...
  Stream<List<Link>> get todayLinks =>
      links.map((links) => links.where((link) => link.isToday).toList());
  ...
  void _invokeSearch(String query) => _searchedLinksController.add(_links
      .where((link) => link.uri.toLowerCase().contains(query.toLowerCase()))
      .toList());
